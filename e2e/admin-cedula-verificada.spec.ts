...
//
// Si alguien vuelve a apuntar el badge a `users.verified`, este spec se pone rojo: doc1 y doc2
// tienen el MISMO users.verified (true) y distinto doctors.verified.
import { test, expect, type Locator, type Page } from '@playwright/test'

/** La fila de esa persona, tras filtrar. Devolver la fila (y no la página) es lo que evita que las
 *  aserciones caigan sobre los ~3500 usuarios sin filtrar mientras corre el debounce. */
...
test('la lista del admin distingue cédula validada de cédula sin validar', async ({ browser }) => {
  ...
  // doc2: MISMO users.verified (true), pero doctors.verified = false. Que estas dos filas se vean
  // distintas es justo lo que no pasaba antes.
  const dos = await filaDe(page, 'E2E Doctor Dos')
  await expect(dos.getByText('Cédula sin verificar')).toBeVisible()
  await expect(dos.getByText('Cédula verificada')).toHaveCount(0)
